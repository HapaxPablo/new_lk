...
function isRussianCoordinates([lng, lat]: [number, number]) {
  return lat >= 41 && lat <= 82 && lng >= 19 && lng <= 191
}
...
export function parseCoordinates(
  ...
  const swappedResult: [number, number] = [latitude, longitude]

  if (isRussianCoordinates(result)) return result
  if (isRussianCoordinates(swappedResult)) return swappedResult
...
export function toMapMarker(input: MapMarkerInput): MapMarker | null {
  ...
  return {
    ...
    imageUrl:
      ...
      undefined,
    logoUrl:
      input.logoUrl ?? input.brandUrl ?? input.brand?.logotype ?? undefined,
    href:
      input.href ??
