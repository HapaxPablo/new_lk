...
export const BrandNomenclatures = ({ brandId }: Props) => {
  const { items, totalCount, isLoadingInitial } = useBrandNomenclatures(brandId)
  ...
  const allSelected =
    items.length > 0 && items.every((item) => ids.includes(item.id))
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.header}>
        ...
        <Button variant="add" onClick={handleSelectAll} isActive={allSelected}>
